...
function z_mini_menu_item_display( $args ) {
    ...
    echo '<input type="checkbox" id="' . $name . '" name="z_mini_menu_plugin_options[' . $name . ']" value="1"' . checked( 1, $ {
        $name
    }, false ) . '/><label for="' . $name . '"> ' . $label . '</label>';
    ...
    if ( !empty( $options[ $name ] ) ) {
        foreach ( $options[ $name ] as $item_key => $item ) {
            ...
            if ( isset( $options[ $name ][ $item_key ][ 'name' ] ) ) {
                $html .= esc_attr( $options[ $name ][ $item_key ][ 'name' ] );
            }
            $html .= '"/></p>';
            ...
            if ( isset( $options[ $name ][ $item_key ][ 'url' ] ) ) {
                $html .= esc_attr( $options[ $name ][ $item_key ][ 'url' ] );
            }
            $html .= '"/></p>';
            ...
            if ( isset( $options[ $name ][ $item_key ][ 'icon' ] ) ) {
                $html .= esc_attr( $options[ $name ][ $item_key ][ 'icon' ] );
            }
            $html .= '"></span>';
...
function z_mini_menu_add_admin_scripts( $hook ) {
    if ( is_admin() ) {
        ...
        wp_enqueue_style( 'z-mini-menu-admin-styles', $admin_css, array( 'dashicons', 'wp-color-picker' ), '1.0' );
